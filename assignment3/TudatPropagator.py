...
def propagate_state_and_covar_back(Xo, Po, tvec, state_params, int_params, bodies=None, alpha=1e-4):
    '''
    This function propagates an orbit using physical parameters provided in
    state_params and integration parameters provided in int_params. It also
    propagates the Gaussian covariance matrix using the Unscented transform.

    This version propagates backwards in time from the last point of tvec to the
    first point of tvec.
    ...
    Xo : 6x1 numpy array
        Cartesian state vector [m, m/s]
    tvec : list or numpy array
        Only the first and last terms are used; the propagation starts at the last
        time (tvec[-1]) and goes backward to the first time (tvec[0]).
    ...
    bodies : tudat object, optional
        contains parameters for the environment bodies used in propagation.
        If None, bodies will be initialized using state_params.

    alpha: float, optional (default=1e-4)
        UKF sigma point spread parameter, typically in range [1e-4, 1]

    Returns
    ------
    tf : float
        final time (the earliest time in tvec) in seconds since J2000
    Xf : nx1 numpy array
        Cartesian state at final time [m, m/s]
    Pf : nxn numpy array
        Gaussian covariance matrix at final time [m^2, m^2/s^2]
    ...
    # The integration will use these sigma points
    initial_state = chi_v.flatten()

    # Retrieve input parameters from state_params
    central_bodies = state_params['central_bodies']
    bodies_to_create = state_params['bodies_to_create']
    mass = state_params['mass']
    Cd = state_params['Cd']
    ...
    # For backward propagation, set:
    #   simulation_start_epoch: starting time at the end of tvec
    #   simulation_end_epoch: the earlier time in tvec
    simulation_start_epoch = tvec[-1]
    simulation_end_epoch = tvec[0]

    # Setup bodies if not provided
    if bodies is None:
        bodies = tudat_initialize_bodies(bodies_to_create)

    # Create the bodies to propagate.
    # TUDAT always uses a 6 element state vector.
    N = int(len(initial_state) / n)
    central_bodies = central_bodies * N
    bodies_to_propagate = []
    for jj in range(N):
    ...
    # Create termination settings based on the earlier time (simulation_end_epoch)
    termination_condition = propagation_setup.propagator.time_termination(
        simulation_end_epoch, terminate_exactly_on_final_condition=True
    )

    # Create numerical integrator settings with negative step sizes for backward propagation.
    if int_params['tudat_integrator'] == 'rk4':
        fixed_step_size = -abs(int_params['step'])
        integrator_settings = propagation_setup.integrator.runge_kutta_4(
            fixed_step_size
    ...
    # Extract the resulting state history and convert it to an ndarray.
    # In backward propagation, the states will be ordered from simulation_start_epoch
    # down to simulation_end_epoch.
    states = dynamics_simulator.state_history
    states_array = result2array(states)

    tf = states_array[0, 0]  # This should now equal simulation_start_epoch!
    chi_v = states_array[0, 1:]

    # Compute mean and covariance at final time using the unscented transform.
    chi = np.reshape(chi_v, (n, 2 * n + 1), order='F')
    mean = np.dot(chi, Wm.T)
    mean = np.reshape(mean, (n, 1))
    chi_diff = chi - np.dot(mean, np.ones((1, 2 * n + 1)))
    ...
    # Output the state vector (first 6 elements) at simulation_start_epoch!
    Xf = chi_v[0:6].reshape(6, 1)
