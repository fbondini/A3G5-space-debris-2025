...
def propagate_state_and_covar(Xo, Po, tvec, state_params, int_params, bodies=None, alpha=1e-4):
    ...
    # [ ]: Note that this will work only for 6 element state vector, if other
...
def propagate_state_and_covar_full(Xo, Po, tvec, state_params, int_params, bodies=None, alpha=1e-4):
    '''
    WITH ALL PARAMETERS
    This function propagates an orbit using physical parameters provided in 
    state_params and integration parameters provided in int_params. It also
    ...
    chi_v = np.reshape(chi, (n*(2*n+1), 1), order='F')
    chi_v_flat = chi_v.flatten()

    # retrieve the initial state and other parameters
    initial_state = np.empty([2 * n + 1, 6])
    Cd = np.empty([2 * n + 1])
    Cr = np.empty([2 * n + 1])
    for i in range(2 * n + 1):
        initial_state[i,:] = chi_v_flat[8 * i:8 * i + 6]
        Cd[i] = chi_v_flat[8 * i + 6] # if chi_v_flat[8 * i + 7] > 0 else 1e-4
        Cr[i] = chi_v_flat[8 * i + 7] # if chi_v_flat[8 * i + 7] > 0 else 1e-4
    ...
    for jj in range(N):
        ...
        if Cr[jj] > 0.:
            ...
            radiation_pressure_settings = environment_setup.radiation_pressure.cannonball_radiation_target(
                area, Cr[jj], occulting_bodies_dict )
    ...
    tf = states_array[-1,0]
    chi_states = states_array[-1,1:]
    chi_v = np.empty(n * (2*n + 1))
    for i in range(2 * n + 1):
        chi_v[i * n:i * n + n] = np.append(chi_states[i * 6: i * 6 + 6].copy(), [Cd[i], Cr[i]])
    ...
    # Output single mean state not computed average of sigma points
    Xf = chi_v[0:8].reshape(8,1)
    
    return tf, Xf, Pf


def propagate_orbit_wdepvars(Xo, tvec, state_params, int_params, bodies=None, depvars=None):
    '''
    This function propagates an orbit using physical parameters provided in 
    state_params and integration parameters provided in int_params.
    ...
    depvars : list[propagation_setup.dependent_variable.SingleDependentVariableSaveSettings], optional
        Dependent variables to save, if none doesn't save any.
    ...
    return tout, Xout, dependent_variable_history
